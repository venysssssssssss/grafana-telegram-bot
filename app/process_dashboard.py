...
def process_dashboard(
    ...
    try:
        if initial_run:
            logger.info(f"Acessando o URL do {dashboard_name}: {dashboard_url}")
            driver.get(dashboard_url)
            time.sleep(5)  # Espera para garantir o carregamento completo da página

            if dashboard_name == 'mvp1':
                logger.info(f"Iniciando autenticação para o {dashboard_name}")
                auth = Authenticator(driver)
                auth.authenticate()
                logger.info(f"Autenticação concluída com sucesso para {dashboard_name}")
            else:
                logger.info(f"Dashboard {dashboard_name} não requer autenticação.")
        else:
        ...
        logger.error(f"Elemento não encontrado durante o acesso ao {dashboard_name}: {e}")
        return None
    except WebDriverException as e:
        logger.error(f"Erro de WebDriver ao acessar {dashboard_name}: {e}")
        return None
    except Exception as e:
        logger.error(f"Erro inesperado durante acesso ao {dashboard_name}: {e}")
        return None
    ...
    try:
        ...
        if result:
            ...
            logger.info(f"Caminho do relatório para {dashboard_name}: {relatorio_path}")

            if initial_run and relatorio_path:
                try:
                    logger.info(f"Enviando mensagem informativa para {dashboard_name}.")
                    ...
                    logger.info(f"Mensagem enviada com sucesso para {dashboard_name}.")
                    ...
                    logger.error(f"Erro ao enviar mensagem ou remover arquivo: {e}")
