...
def scroll_table_to_bottom(driver):
    ...
    for _ in range(100):  # Realiza a rolagem 3 vezes
        try:
            ...
            action.move_by_offset(0, 3).perform()  # Ajuste este valor conforme necessário
            action.release().perform()
            ...
            total_rows_counted += new_rows_count  # Atualiza o contador total de linhas

            # Coleta as informações das novas linhas
            for row in range(total_rows_counted - new_rows_count + 1, total_rows_counted + 1):
                try:
...
def monitor_falhas(driver):
    while True:
        count_success, count_business_error, count_system_failure, falha_detectada, _, _, _, total_rows = collect_info(driver)
        
        if falha_detectada:
            send_telegram_message("Falha de sistema\n\nℹ️ Informação: falha ao importar pedidos")
            while falha_detectada:
                time.sleep(60)
                _, _, _, falha_detectada, _, _, _, _ = collect_info(driver)
